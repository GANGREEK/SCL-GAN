...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=9, padding_type='zero', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        model = [#nn.ReflectionPad2d(3),
                 nn.Conv2d(input_nc, ngf, kernel_size=5, padding=2, bias=use_bias),
        ...
        for i in range(n_downsampling):  # add upsampling layers
            ...
            model += [nn.Upsample(scale_factor=2),
                      #MobileViTBlock(ngf * mult, (ngf * mult)),
                      nn.Conv2d(ngf * mult, int(ngf * mult /2), kernel_size=1, stride=2, padding=0, bias=use_bias),
                      nn.Upsample(scale_factor=2),     
                      nn.InstanceNorm2d(int(ngf * mult / 2)),
                      nn.ReLU(True)]
        #model += [nn.ReflectionPad2d(3)]
        model += [nn.Conv2d(ngf, output_nc, kernel_size=5, padding=2)]
        model += [nn.Tanh()]
...
class MobileResnetBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, dropout_rate, use_bias):
        ...
        conv_block += [
            ...
            AdaptiveInstanceNorm2d(dim), nn.ReLU(True)
        ]
...
class SeparableConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, norm_layer=nn.InstanceNorm2d,
        ...
        #self.ATT = Attention_block(F_g = int(in_channels),F_l=int(in_channels),F_int = int(in_channels/2))
    def forward(self, x):
        #for i in range(2):
